...
namespace XReal.XTown.UI
{
    public class UIManager : MonoBehaviour
    {
        ...
        Stack<UIPopup> _popupStack = new Stack<UIPopup>(); //가장 뒤에 추가되는 UI가 가장 먼저 사라져야하니 Stack으로 관리
        ...
        public GameObject Root
        {
            get
            {
                GameObject root = GameObject.Find("@UI_Root"); //@UI_Root이라는 폴더같은 느낌적인 느낌의 친구를 생성해 root부터 시작하도록
        ...
        public T MakeSubItem<T>(Transform parent = null, string name = null) where T : UIBase
        {
            ...
            return UI_Utils.GetOrAddComponent<T>(go);
        }
